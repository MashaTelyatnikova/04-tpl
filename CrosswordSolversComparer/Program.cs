...
namespace CrosswordSolversComparer
{
    public static class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                single.Add(string.Format("{0}",singleThreadedResults[key].Sum()/singleThreadedResults[key].Count));
                multi.Add(string.Format("{0}", multiThreadedResults[key].Sum() / multiThreadedResults[key].Count));
            }
